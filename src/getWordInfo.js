    ...
    try {
        ...
        if (data.definitions.length > 0) {
            ...
            if (data.audio) {
                ...
                console.log(`Anomaly: Word "${word}" has no valid audio id, so it wont get an audio file`);
            }
            ...
            return `Success: Word "${word}" descriptor has been saved to db`;
        } else {
            saveNoDef(word);
            return `Anomaly: Word "${word}" has not any definitions, so it wont be stored in the db`;
        }
    } catch (error) {
